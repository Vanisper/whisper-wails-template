<template>
    <transition name="dialog">
        <div v-if="show" class="dialog" :class="getDialogClass"
        ...
        >
            ...
            >
                ...
                <div :class="loading && 'loading'" class="body">
                    <slot />
                </div>
                <div v-if="!hideFooter" class="footer">
                    ...
                    <div v-if="!hideCtrl" class="control">
                        <slot name="leftCtrl" />
                        <AButton v-if="!hideConfirm" primary :disabled="disableConfirm || loading" @click="confirmEvent">{{ confirmText }}</AButton>
                        <slot name="middleButton" />
                        <AButton v-if="!hideCancel" @click="emits('onCancel')">{{ cancelText }}</AButton>
                    </div>
                </div>
            </div>
        </div>
...
import { PropType, computed, nextTick, onMounted, ref, watch, toRef } from 'vue';
import { FormInstance } from '@arco-design/web-vue';
import { CoreNotification } from '@/utils/feedback/notification';
import { CoreRand } from '@/utils/core-rand';
...
async function confirmEvent() {
    ...
    // 校验抛出了一异常
        ...
        if (keys.length > 0) {
            ...
            const list = ((e as any)[keys[0]] as any);
            if (list.length > 0) {
