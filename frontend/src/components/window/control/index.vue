<template>
  ...
  <div title="最小化" class="frameless-titlebar-button frameless-titlebar-minimize" @click="minimizeWindow">
    <svg width="10" height="10">
      <path v-for="(d, _index) in minimizePath" :d="d" :key="_index" />
    </svg>
  </div>
  <div title="最大化" class="frameless-titlebar-button frameless-titlebar-toggle" @click="maximiseWindow">
    <svg width="10" height="10">
      <path v-for="(d, _index) in (isMaximised ? restorePath_win11 : maximizePath_win11)" :d="d" :key="_index" />
    </svg>
  </div>
  <div title="关闭" class="frameless-titlebar-button frameless-titlebar-close" @click="closeWindow">
    <svg width="10" height="10">
      <path v-for="(d, _index) in closePath" :d="d" :key="_index" />
    </svg>
  </div>
</template>
...
const isMaximised = ref<boolean>();

window.onresize = async (_event: any) => {
