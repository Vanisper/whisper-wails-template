            ...
            <slot :name="item.key" :data="(scope as any).row" :index="(scope as any).$index">
              ...
              <div v-if="item.dictionary || getDictionary(entityInstance, item.key)" class="status">
                ...
                <span v-if="item.showColor" class="light" :style="{
                  backgroundColor:
                    (item.dictionary || getDictionary(entityInstance, item.key) || new CoreDictionaryArray())
                      .getColor((scope as any).row[item.key], CoreColor.NORMAL)
                }" />
                {{
                  (item.dictionary || getDictionary(entityInstance, item.key) || new CoreDictionaryArray())
              ...
              <template v-else-if="item.image">
                <el-image style="background-color:#f3f6f9"
                  ...
                  :src="CoreFile.getStaticFileUrl((scope as any).row[item.key])"
                  :preview-src-list="[CoreFile.getStaticFileUrl((scope as any).row[item.key])]" :z-index="999999"
          ...
          <div class="ctrlRow">
            ...
            <template v-if="!hideCtrl">
              <AButton v-if="showAdd" icon-button type="ADD" :disabled="isAddDisabled((scope as any).row)"
                ...
                :permission="addPermission || CorePermission.getPermission(entity, CorePermissionAction.ADD_CHILD)"
              ...
              <AButton v-if="!hideEdit" icon-button type="EDIT" :disabled="isEditDisabled((scope as any).row)"
                ...
                :permission="editPermission || CorePermission.getPermission(entity, CorePermissionAction.EDIT)"
              ...
              <AButton v-if="showDetail" icon-button type="DETAIL" :disabled="isDetailDisabled((scope as any).row)"
                ...
                :permission="detailPermission || CorePermission.getPermission(entity, CorePermissionAction.DETAIL)"
              ...
              <AButton v-if="!hideDelete" icon-button type="DELETE" :danger="isForceDelete"
                ...
                :permission="deletePermission || CorePermission.getPermission(entity, CorePermissionAction.DELETE)"
...
const props = defineProps({
   ...
   */
  dataList: {
    type: Array as PropType<CoreEntity[]>,
    required: true,
  },
   ...
   */
  fieldList: {
    type: Array as PropType<CoreTableFieldConfig[]>,
    default: () => [],
  },
 ...
 * # 显示按下快捷键的提醒
 */
watch(() => CoreStore().controllKeyDown, () => {
  isForceDelete.value = false
  if (
    CoreStore().controllKeyDown
    && !props.customDelete
    && !props.hideDelete
    && props.dataList
...
const allFieldList = computed(() => {
  ...
  return (entityInstance.value.getTableFieldConfigList().filter((item) => !item.removed) || []).map((item) => {
    if (item.dictionary) {
      item.dictionary = CoreDictionaryArray.create(item.dictionary)
    }
    return item
  })
})
...
function updateSelectedFieldList() {
  ...
  const fieldListCache: string[] = JSON.parse(localStorage.getItem(`field_list_of_${CoreConfig.appKey}_${entityInstance.value.constructor.name}`) || '[]')
 ...
 * @param config 配置
 */
function fieldSelectChanged(status: boolean, config: CoreTableFieldConfig) {
  if (config.forceShow) {
    return
  }
  ...
  localStorage.setItem(`field_list_of_${CoreConfig.appKey}_${entityInstance.value.constructor.name}`, JSON.stringify(selectedFieldList.value))
}
 ...
 * 字段是否选择
 */
function isSelected(item: CoreTableFieldConfig) {
  if (!item.key) {
    return false
  }
...
function getPayloadRowData(row: any, config: CoreTableFieldConfig) {
 ...
 * 表格dom
 */
const airTableRef = ref<CoreTableInstance>()
 ...
 * @param item 行数据
 */
function handleAdd(item: CoreEntity) {
  emits('onAdd', item)
}
 ...
 * @param item 详情数据
 */
function handleDetail(item: CoreEntity) {
  emits('onDetail', item)
}
 ...
 * @param item 编辑的数据
 */
function handleEdit(item: CoreEntity) {
  emits('onEdit', item)
}
 ...
 * @param item
 */
async function handleDelete(item: CoreEntity) {
  if (props.customDelete) {
    emits('onDelete', item)
    return
  }
  try {
    if (!CoreStore().controllKeyDown) {
      let title = '删除提醒'
      ...
      await CoreConfirm.create().dangerButton().enableEscClose().setConfirmText('确认删除')
        .show(content, title)
    }
    emits('onDelete', item)
  } catch (e) {
...
function sortChanged(data: { prop: string; order: string }) {
  if (data.prop) {
    const sort = new CoreSort()
    sort.field = data.prop
    sort.direction = data.order === 'descending' ? CoreSortType.DESC : CoreSortType.ASC
    emits('onSort', sort)
  } else {
