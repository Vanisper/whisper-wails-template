<template>
  <div class="air-page">
    <el-pagination v-model:currentPage="page.pageNum" v-model:page-size="page.pageSize" class="air-page-bar" background
      :page-sizes="CoreConfig.pageSizes" layout=" prev, next" :total="response.total" small
      @current-change="pageChanged($event)" />
    <el-popover v-if="page.pageNum && response.pageCount" :width="226" trigger="click" @hide="pageChanged(page.pageNum)">
      <template #reference>
        ...
        <div class="air-page-panel-box">
          <div class="air-page-header">
            ...
            <el-radio-group v-model="page.pageSize" class="air-page-radio" size="small">
              <el-radio-button v-for="item in CoreConfig.pageSizes" :key="item" :label="item" @click="sizeChanged(item)">
                {{ item }}
              </el-radio-button>
            </el-radio-group>
          </div>
          <div class="air-page-goto">
            <el-button v-for="item in pageCountList" :key="item" round
              :type="page.pageNum == item ? 'primary' : 'default'" @click="pageChanged(item)">
              {{ item }}
            </el-button>
          </div>
          <div class="air-page-jumper">
            <el-input v-model="page.pageNum" type="number" placeholder="输入页码跳转" min="1" :max="response.pageCount">
              <template #append>
                <el-button size="small" @click="pageChanged(page.pageNum)">
                  跳转
                </el-button>
              </template>
            </el-input>
...
const props = defineProps({
   ...
   */
  response: {
    type: CoreResponsePage,
    required: true,
  },
})

const page = ref(new CorePage())
