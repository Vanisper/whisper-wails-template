<template>
  <transition name="dialog">
    <div class="dialog air-dialog" :class="getDialogClass" @mouseup="dialogMouseUpEvent" @mousemove="dialogMouseMoveEvent"
      ...
      <button :id="'hidden-button-' + randId" class="hidden-button" />
      <div :id="randId" class="main" :style="{
        ...
        <div v-if="!hideFooter" class="footer">
          ...
          <div v-if="!hideCtrl" class="control">
            <slot name="leftCtrl" />
            <AButton v-if="!hideConfirm" primary :disabled="disableConfirm || loading" @click="confirmEvent">
              {{ confirmText }}
            </AButton>
            <slot name="middleButton" />
            <AButton v-if="!hideCancel" @click="emits('onCancel')">
              {{ cancelText }}
            </AButton>
          </div>
        </div>
...
import type { CoreFormInstance } from '../type/CoreType';
import { CoreRand } from '../helper/CoreRand';
...
const props = defineProps({
   ...
   * # 隐藏取消按钮
   * -
   * 💡 默认为 ```CoreConfig.dialogHideCancel```
   */
    hideCancel: {
        type: Boolean,
        default: CoreConfig.dialogHideCancel,
    },
   ...
   */
    fullable: {
        type: Boolean,
        default: CoreConfig.dialogFullable,
    },
   ...
   */
    formRef: {
        type: Object as PropType<CoreFormInstance>,
        default: undefined,
    },
   ...
   * # 是否支持点击遮罩层关闭
   * ---
   * 默认值 ```CoreConfig.dialogCloseByCover = false```
   */
    hoverClose: {
        type: Boolean,
        default: CoreConfig.dialogCloseByCover,
    },
});
 ...
 * # 随机ID
 */
const randId = ref(`id${CoreRand.getRandNumberAndCharString()}`);
...
watch(() => CoreStore().escKeyDown, () => {
    if (CoreStore().escKeyDown && CoreConfig.dialogCloseByEsc) {
        emits('onCancel');
    }
});
...
async function confirmEvent() {
    ...
    // 校验抛出了一异常
        ...
        if (keys.length > 0) {
            ...
            const list = ((e as any)[keys[0]] as CoreValidator[]);
            if (list.length > 0) {
                CoreNotification.warning(list[0].message, '验证失败');
            }
        }
        dialogBgClicked();
        return;
