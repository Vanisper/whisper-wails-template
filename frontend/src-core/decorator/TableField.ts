 ...
 * @param key 属性名
 */
export function getTableConfig(target: any, key: string): CoreTableFieldConfig | null {
...
export function getTableConfigList(target: any, keyList: string[]) {
    return CoreDecorator.getFieldConfigList(target, FIELD_LIST_KEY, FIELD_CONFIG_KEY, keyList, CoreTableFieldConfig).sort((a, b) => b.orderNumber - a.orderNumber).map((item) => {
