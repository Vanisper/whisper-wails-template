...
import { CoreFieldConfig } from '../config/CoreFieldConfig'
...
export class CoreDecorator {
  ...
  static getClassConfig(target: any, classConfigKey: string, defaultValue: any = undefined, isObject = false): any {
    ...
    if (!isObject) {
      ...
      if (classConfig === undefined) {
        ...
        if (!superClass || superClass.constructor.name === 'CoreModel') {
          return undefined
        }
   ...
   */
  static getFieldConfigList<T extends CoreFieldConfig>(target: any, fieldListKey: string, fieldConfigKey: string, keyList: string[], FieldConfigClass: ClassConstructor<T>) {
  ...
  static getFieldConfigValue(target: any, fieldConfigKey: string, key: string, configKey: string): any {
    const fieldConfig = CoreClassTransformer.copyJson(Reflect.get(target, `${`${fieldConfigKey}[${key}]`}`))
